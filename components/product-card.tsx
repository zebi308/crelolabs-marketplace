...
export function ProductCard({ product, onPreview }: { product: Product; onPreview?: (product: Product) => void }) {
  return (
    <motion.article
      whileHover={{ y: -6 }}
      className="group relative overflow-hidden rounded-lg border border-white/10 bg-white/[0.055] p-4 shadow-luxe backdrop-blur-xl"
    >
      <div className={`relative mb-4 h-44 overflow-hidden rounded-md bg-gradient-to-br ${product.accent}`}>
        ...
        <div className="absolute bottom-4 left-4 right-4">
          ...
          <h3 className="mt-3 line-clamp-2 text-xl font-semibold tracking-tight text-white">{product.title}</h3>
        </div>
      </div>
      <div className="flex items-center justify-between gap-4">
        <Badge>{product.category}</Badge>
        <span className="inline-flex items-center gap-1 text-sm text-white/[.68]">
          <Star className="h-4 w-4 fill-blue-300 text-blue-300" /> {product.rating}
        </span>
      </div>
      <p className="mt-4 line-clamp-2 min-h-[48px] text-sm leading-6 text-white/[.56]">{product.description}</p>
