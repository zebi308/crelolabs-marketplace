...
export function Storefront({ mode = "full" }: { mode?: "full" | "featured" }) {
  ...
  return (
    <div>
      {mode === "full" && (
        <div className="mb-8 grid gap-4 lg:grid-cols-[1fr_auto]">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-amber-100/35" />
      ...
      {visible.length === 0 && (
        <div className="rounded-lg border border-dashed border-amber-300/25 p-12 text-center bg-slate-950/30">
          <p className="text-lg font-medium text-amber-100">No matching systems yet.</p>
          <p className="mt-2 text-sm text-amber-100/60">Try a broader search or reset the category filter.</p>
        </div>
      )}
