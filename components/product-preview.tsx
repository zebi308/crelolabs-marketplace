  ...
  return (
    <Dialog.Root open={Boolean(product)} onOpenChange={(open) => !open && onClose()}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-[71] max-h-[88vh] w-[calc(100vw-2rem)] max-w-3xl -translate-x-1/2 -translate-y-1/2 overflow-auto rounded-lg border border-amber-300/20 bg-slate-950 p-5 shadow-luxe">
          {product && (
            <>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                {product.includes.slice(0, 4).map((item) => (
                  <div key={item} className="flex items-center gap-2 rounded-md border border-amber-300/20 bg-slate-950/30 p-3 text-sm text-amber-100/80">
                    <CheckCircle2 className="h-4 w-4 text-amber-300" /> {item}
                  </div>
                ))}
              </div>
          ...
          <Dialog.Close className="absolute right-4 top-4 rounded-md bg-slate-950/40 p-2 text-amber-100/70 hover:text-amber-100" aria-label="Close">
            <X className="h-4 w-4" />
          </Dialog.Close>
        </Dialog.Content>
      </Dialog.Portal>
