  ...
  return (
    <Section eyebrow="Admin" title="A lean control room for digital product operations.">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
        <Card className="p-5">
          <h2 className="mb-4 flex items-center gap-2 text-xl font-semibold text-white"><UploadCloud className="h-5 w-5 text-blue-300" /> Upload product</h2>
          <div className="grid gap-3">
            <Input placeholder="Product title" />
            <Input placeholder="Category" />
            <Input placeholder="Price" />
            <div className="grid min-h-40 place-items-center rounded-lg border border-dashed border-white/[.16] bg-white/5 p-6 text-center text-sm text-white/[.48]">
            ...
            {products.map((product) => (
              <div key={product.slug} className="grid gap-3 p-4 md:grid-cols-[1fr_auto_auto] md:items-center">
                <div>
                  ...
                  <div className="mt-1 text-sm text-white/[.42]">{product.category}</div>
                </div>
        ...
        <Card className="p-5">
          ...
          <div className="mt-6 flex h-64 items-end gap-3">
            {[34, 52, 47, 72, 65, 86, 78, 94].map((height, index) => (
              <div key={index} className="flex-1 rounded-t-md bg-gradient-to-t from-blue-500 to-violet-400" style={{ height: `${height}%` }} />
            ))}
          </div>
        </Card>
        <Card className="p-5">
          ...
          {["maya@studio.com", "andre@creator.co", "sofia@design.io"].map((email) => (
            <div key={email} className="mt-4 flex items-center justify-between rounded-md border border-white/[.08] bg-white/5 p-3">
...
function AdminMetric({ label, value, icon }: { label: string; value: string; icon: React.ReactNode }) {
  return (
    <Card className="p-5">
      <div className="flex items-center justify-between text-blue-300">{icon}</div>
